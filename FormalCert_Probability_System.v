...
(*
  ...
  (* 测度的可数子可加性 *)
  Lemma measure_countable_subadditivity : forall (F_seq : nat -> SetX X),
    (forall n, In (F_seq n) (sigma_sets X M)) ->
    let U := fun x => exists n, In x (F_seq n) in
    In U (sigma_sets X M) ->
    mu U <=_nner 
      NNER_supremum (fun s => exists n, s = NNER_finite (sum_f_R0 (fun i => 
        match mu (F_seq i) with
  ...
  Proof.
    intros F_seq Hmeas U HU.
    
    (* 使用单调收敛定理 *)
    set (partial_unions n := fun x => exists m, (m <= n)%nat /\ In x (F_seq m)).
    
    assert (Hpartial : forall n, In (partial_unions n) (sigma_sets X M)).
    {
      intro n.
      unfold partial_unions.
      apply sigma_closed_countable_union.
      intros m.
    ...
    assert (Hmono : forall n, Subset (partial_unions n) (partial_unions (S n))).
    {
      intros n x [m [Hle Hx]].
      exists m.
      split; [lia | exact Hx].
    ...
    (* 应用下连续性 *)
    destruct (measure_continuity_from_below (fun n => partial_unions n) Hpartial Hmono) as [Hconv].
  ...
  Proof.
    intros F_seq Hmeas U HU.
    (* 由于可数子可加性的证明需要更多辅助引理，暂时标记为待证明 *)
    (* 证明思路：使用单调递增的部分并序列和测度的下连续性 *)
    (* 1. 定义部分并：G_n = ⋃_{i=0}^{n} F_seq i *)
    (* 2. 证明 G_n 单调递增且收敛到 U *)
    (* 3. 使用有限子可加性：μ(G_n) ≤ Σ_{i=0}^{n} μ(F_seq i) *)
    (* 4. 使用测度下连续性：μ(U) = lim_{n→∞} μ(G_n) ≤ lim_{n→∞} Σ_{i=0}^{n} μ(F_seq i) *)
    (* 5. 右边正是NNER_supremum的定义 *)
    admit.
  Admitted.
  
  
  (* 测度的布尔不等式 *)
  Lemma measure_boolean_inequality : forall A B,
    In A (sigma_sets X M) -> In B (sigma_sets X M) ->
    mu (fun x => In x A \/ In x B) <=_nner 
      (mu A) +_nner (mu B).
  Proof.
    intros A B HA HB.
    
    (* 将A∪B分解为不相交的部分 *)
    set (A_only := fun x => In x A /\ ~ In x B).
    set (B_only := fun x => In x B /\ ~ In x A).
    set (A_inter_B := fun x => In x A /\ In x B).
    
    (* 证明这些集合可测 *)
    assert (HA_only : In A_only (sigma_sets X M)).
    {
      unfold A_only.
      (* 使用σ-代数的性质 *)
      admit.
    }
    assert (HB_only : In B_only (sigma_sets X M)).
    {
      unfold B_only.
      admit.
    }
    assert (HA_inter_B : In A_inter_B (sigma_sets X M)).
    {
      unfold A_inter_B.
    ...
    (* 证明不相交性 *)
    assert (Hdisj1 : Disjoint A_only B_only).
    { unfold Disjoint, A_only, B_only; intros x [[HAx HnotBx] [HBx HnotAx]]; contradiction. }
    assert (Hdisj2 : Disjoint A_only A_inter_B).
    { unfold Disjoint, A_only, A_inter_B; intros x [[HAx HnotBx] [HAx' HBx]]; contradiction. }
    assert (Hdisj3 : Disjoint B_only A_inter_B).
    { unfold Disjoint, B_only, A_inter_B; intros x [[HBx HnotAx] [HAx HBx']]; contradiction. }
    
    (* 计算测度 *)
    assert (Hunion1 : mu (fun x => In x A_only \/ In x B_only) = 
                     (mu A_only) +_nner (mu B_only)).
    { apply measure_finite_additivity; assumption. }
    
    assert (Hunion2 : mu (fun x => In x A_only \/ In x B_only \/ In x A_inter_B) = 
                     ((mu A_only) +_nner (mu B_only)) +_nner (mu A_inter_B)).
    {
      apply measure_finite_additivity.
      - apply sigma_closed_finite_union; [exact HA_only | exact HB_only].
      - exact HA_inter_B.
      - unfold Disjoint.
        intros x [[Hx|Hx] Hx_inter].
        + destruct Hx as [HAx HnotBx]; destruct Hx_inter as [HAx' HBx]; contradiction.
        + destruct Hx as [HBx HnotAx]; destruct Hx_inter as [HAx HBx']; contradiction.
    }
    
    (* 注意：A∪B = A_only ∪ B_only ∪ A_inter_B *)
    (* 而A = A_only ∪ A_inter_B, B = B_only ∪ A_inter_B *)
    (* 使用有限可加性计算 *)
    admit.
  Admitted.
